...
const ClubsPage: React.FC<ClubsPageProps> = ({ onClubSelect, onGoBack }) => {
  ...
  const fetchClubs = useCallback(async (pageNum: number, searchQuery: string, isNewSearch: boolean) => {
    if (isNewSearch) {
        setIsLoading(true);
    } else {
    ...
    try {
        ...
        setClubs(prev => isNewSearch ? newClubs : [...prev, ...newClubs]);
  ...
  useEffect(() => {
    ...
    setPage(1);
    setHasMore(true);
    fetchClubs(1, debouncedQuery, true);
  }, [debouncedQuery, fetchClubs]);

  const loadMore = () => {
    if (!isLoadingMore && hasMore) {
        fetchClubs(page + 1, debouncedQuery, false);
    }
  };
             ...
             <>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {isLoading && clubs.length === 0 ? (
                        ...
                        clubs.map(club => <ClubCard key={club.mal_id} club={club} onSelect={onClubSelect} />)
                    ) : !isLoading ? (
