...
const ClubDetailPage: React.FC<ClubDetailPageProps> = ({ club, onGoBack, onSelectAnime, getEpisodeStatus, onLoginRequest }) => {
                        ...
                        {relations?.anime && relations.anime.length > 0 && (
                            <div>
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
                                    {relations.anime.map(item => <AnimeCard key={item.mal_id} anime={mapPartialToFullAnime({ id: item.mal_id, title: item.name, thumbnail: item.images?.jpg.image_url || '' })} onSelect={onSelectAnime} episodeStatus={getEpisodeStatus(item.mal_id)} onLoginRequest={onLoginRequest} />)}
                                </div>
                            </div>
                        )}
